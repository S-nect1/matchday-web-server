...
//    }
    ...
    @Nested
    @DisplayName("validateBothPermission")
    class ValidateBothPermissionTest {

        final Long HOME_ID = 10L;
        final Long AWAY_ID = 20L;
        ...
        @BeforeEach
        void initTeamsAndUser() {
            home = TeamFixture.teamWithName("home");
            away = TeamFixture.teamWithName("away");
            user = UserFixture.defaultUser1();
            ReflectionTestUtils.setField(home, "id", HOME_ID);
            ReflectionTestUtils.setField(away, "id", AWAY_ID);
        }

        @Test
        @DisplayName("홈 운영 권한이 있으면 통과")
        void pass_whenHomeHasAuthority() {
            TeamUser homeManager = createTeamUser(home, user, TeamRole.MANAGER);

            given(teamUserRepository.findByTeamIdAndUserId(eq(HOME_ID), eq(USER_ID)))
                    .willReturn(Optional.of(homeManager));
            given(teamUserRepository.findByTeamIdAndUserId(eq(AWAY_ID), eq(USER_ID)))
                    .willReturn(Optional.empty());

            assertThatCode(() ->
                    teamUserService.validateBothPermission(HOME_ID, AWAY_ID, USER_ID)
            ).doesNotThrowAnyException();

            verify(teamUserRepository).findByTeamIdAndUserId(HOME_ID, USER_ID);
            verify(teamUserRepository).findByTeamIdAndUserId(AWAY_ID, USER_ID);
        }

        @Test
        @DisplayName("어웨이 운영 권한이 있으면 통과")
        void pass_whenAwayHasAuthorityAndHomeEmpty() {
            TeamUser awayLeader = createTeamUser(away, user, TeamRole.LEADER);

            given(teamUserRepository.findByTeamIdAndUserId(eq(HOME_ID), eq(USER_ID)))
                    .willReturn(Optional.empty());
            given(teamUserRepository.findByTeamIdAndUserId(eq(AWAY_ID), eq(USER_ID)))
                    .willReturn(Optional.of(awayLeader));
        ...
        @Test
        @DisplayName("두 팀 모두 소속 아닌 경우 예외")
        void throws_whenNotBelongToEither() {
            given(teamUserRepository.findByTeamIdAndUserId(eq(HOME_ID), eq(USER_ID)))
                    .willReturn(Optional.empty());
            given(teamUserRepository.findByTeamIdAndUserId(eq(AWAY_ID), eq(USER_ID)))
                    .willReturn(Optional.empty());
            ...
            assertThatThrownBy(() ->
                    ...
                    .isInstanceOf(TeamControllerAdvice.class)
                    .hasMessage(ResponseCode.TEAM_USER_NOT_FOUND.getMessage());
        }

        @Test
        @DisplayName("양쪽 모두 소속이지만 둘 다 권한 없는 경우 예외")
        void throws_whenBothBelongButNoAuthority() {
            TeamUser homeMember = createTeamUser(home, user, TeamRole.MEMBER);
            TeamUser awayMember = createTeamUser(away, user, TeamRole.MEMBER);

            given(teamUserRepository.findByTeamIdAndUserId(eq(HOME_ID), eq(USER_ID)))
                    .willReturn(Optional.of(homeMember));
            given(teamUserRepository.findByTeamIdAndUserId(eq(AWAY_ID), eq(USER_ID)))
                    .willReturn(Optional.of(awayMember));

            assertThatThrownBy(() ->
                    teamUserService.validateBothPermission(HOME_ID, AWAY_ID, USER_ID)
            )
                    .isInstanceOf(TeamControllerAdvice.class)
                    .hasMessage(ResponseCode._UNAUTHORIZED.getMessage());
        }
    }
}